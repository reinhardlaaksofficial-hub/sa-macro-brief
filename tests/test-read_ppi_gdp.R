...
test_that("read_ppi_file reproduces the June 2026 release values", {
  long <- read_ppi_file(ppi_paths$main, period = "2026-06", root = proj_root)
  ...
  expect_true("PPC30000" %in% long$series_code)
})

test_that("read_gdp_file reproduces the Q1 2026 release values", {
  long <- suppressMessages(read_gdp_file(gdp_paths$main, period = "2026Q1", root = proj_root))
...
test_that("the duplicated 201803 column is repaired to 2018Q4, not dropped", {
  expect_message(read_gdp_file(gdp_paths$main, root = proj_root), "2018Q4")
  long <- suppressMessages(read_gdp_file(gdp_paths$main, root = proj_root))
...
test_that("GDP contributions reconcile with the headline within tolerance", {
  long <- suppressMessages(read_gdp_file(gdp_paths$main, root = proj_root))
  d <- list(gdp = long, vintage = list(list(retrieved_at = "2026-08-26 00:00:00 UTC")))
  t <- transform_gdp(d, "2026Q1", root = proj_root)
